...
def toggle_group_mode(enable: bool) -> str:
    """
    Toggles Group Mode (Discord Loopback). When enabled, Jarvis will hear the system desktop audio in addition to the microphone.
    Call this when the user says "open comms", "listen to Discord", "enable group mode", or similar.
    Args:
        enable: True to turn on Group Mode, False to turn it off.
    """
    import audio_stream
    audio_stream.set_group_mode(enable)
    if enable:
        return "Group Mode enabled. You can now hear system audio and friends on Discord."
    else:
        return "Group Mode disabled. You are now only listening to the microphone."
...
def click_vision_element(description: str) -> str:
    """
    Uses OpenAI Vision to find the exact X,Y coordinates of an element on the screen matching the description and clicks it.
    Use this to click on buttons/elements inside a web browser (like Facebook 'Home' button).
    Args:
        description: A very clear description of the button/element to click (e.g. 'The blue Add Friend button').
    """
    import pyautogui
    from datetime import datetime
    import base64
    ...
    try:
        temp_dir = tempfile.gettempdir()
        filename = f"jarvis_vision_click_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
        path = os.path.join(temp_dir, filename)
        
        # Take screenshot
        img = pyautogui.screenshot()
        width, height = img.size
        # Resize for faster API upload
        ...
        # Prompt GPT to return JSON with X,Y
        prompt = (
            f"Find the element on the screen matching this description: '{description}'. "
            f"The image provided is a screenshot resized to {width//2}x{height//2}. "
            f"Return ONLY a JSON object with 'x' and 'y' representing the center coordinates of the element in the resized image. "
        ...
        print(f"[LLM Vision] Searching for '{description}' to click...")
        response = requests.post("https://api.openai.com/v1/chat/completions", headers=headers, json=payload)
        response.raise_for_status()
        ...
        x_small, y_small = data.get("x"), data.get("y")
        if x_small is None or y_small is None:
            return "Vision model returned invalid coordinates."
            
        # Scale back to original screen resolution
        x_actual = x_small * 2
        y_actual = y_small * 2
        
        # Move and click
        pyautogui.moveTo(x_actual, y_actual, duration=0.3)
        time.sleep(0.1)
        pyautogui.click()
        
        return f"Successfully clicked on '{description}' at ({x_actual}, {y_actual})."
        
    except Exception as e:
        return f"Error executing vision click: {e}"
