...
def run_ui():
    ...
    try:
        ...
        win32gui.SetWindowLong(hwnd, win32con.GWL_EXSTYLE,
                               exstyle | win32con.WS_EX_LAYERED | 0x00000080)
    ...
    while True:
        ...
        # ---------------------------------------------------------------
        # Window position — smooth lerp to target per state
        # ---------------------------------------------------------------
        if   state in ('booting', 'speaking', 'thinking'):
            tx = screen_w / 2 - WIDTH / 2
            ty = screen_h / 2 - HEIGHT / 2
        elif state == 'idle':
            tx = screen_w - WIDTH - 40
            ty = 40
        elif state == 'listening':
            tx = screen_w / 2 - WIDTH / 2
            ty = screen_h - HEIGHT - 80
        else:
            tx, ty = cur_x, cur_y
        ...
        if hwnd:
            ...
            if should_hide != last_hide:
                ...
                _wg.ShowWindow(hwnd, _wc.SW_HIDE if should_hide else _wc.SW_SHOW)
            if not should_hide:
                import win32con as _wc, win32gui as _wg
                _wg.SetWindowPos(hwnd, _wc.HWND_TOPMOST,
                                 int(cur_x), int(cur_y), 0, 0, _wc.SWP_NOSIZE)
